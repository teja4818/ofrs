<?php
 ...
 session_start();
 $username = $_SESSION["name1"];/* userid of the user */
 $con = mysqli_connect('localhost','root','','ofrs') or die('Unable To connect');
 if(count($_POST)>0) {
 $result = mysqli_query($con,"SELECT *from adminlogin WHERE username='" . $username . "'");
 $row=mysqli_fetch_array($result);
 if($_POST["currentPassword"] == $row["password"] && $_POST["newPassword"] == $_POST["confirmPassword"] ) {
 mysqli_query($con,"UPDATE adminlogin set password='" . $_POST["newPassword"] . "' WHERE username='" . $username . "'");
